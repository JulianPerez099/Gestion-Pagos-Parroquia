...
<script>
    ...
    $('#manage-course').submit(function(e) {
        ...
        if ($('#fee-list tbody').find('[name="fid[]"]').length <= 0) {
            alert_toast("Inserte información en la tabla tarifas", 'danger')
            end_load()
            return false;
        }
        $.ajax({
            ...
            success: function(resp) {
                if (resp == 1) {
                    ...
                    $('#msg').html('<div class="alert alert-danger mx-2">El Año ya existe.</div>')
                    end_load()
                }
            }
        })
